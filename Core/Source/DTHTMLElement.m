...
- (NSAttributedString *)attributedString
{
	@synchronized(self)
	{
		...
		else
		{
			...
			for (DTHTMLElement *oneChild in self.childNodes)
			{
                ...
                if (previousChild) {
                    if (previousChild.displayStyle == DTHTMLElementDisplayStyleInline) {
    ...
    //                        }
                            // paragraph break
                            if (tmpString.length > 0 && ![[tmpString string] hasSuffixCharacterFromSet:[NSCharacterSet ignorableNewlineCharacterSet]]) {
                                [tmpString appendString:@"\n"];
                            }
                        }
                    } else {
                        if (oneChild.displayStyle == DTHTMLElementDisplayStyleInline) {
                            if (tmpString.length > 0 && [[tmpString string] hasSuffixCharacterFromSet:[NSCharacterSet ignorableNewlineCharacterSet]]) {
...
//						if ([[tmpString string] hasSuffixCharacterFromSet:[NSCharacterSet ignorableNewlineCharacterSet]])
//						{
//							// following e.g. a BR we don't want a space or NL
//							NSCharacterSet *charactersToIgnore = [NSCharacterSet ignorableNewlineCharacterSet];
//                            if ([[nodeString string] hasPrefixCharacterFromSet:charactersToIgnore]) {
//
...
//								if (![field isEqualToString:DTListPrefixField] && ![[nodeString attribute:DTHorizontalRuleStyleAttribute atIndex:0 effectiveRange:NULL] boolValue])
//								{
//                                    nodeString = [nodeString attributedSubstringFromRange:NSMakeRange(1, [nodeString length]-1)];
//								}
//							}
